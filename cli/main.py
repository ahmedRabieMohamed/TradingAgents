...
def get_user_selections():
    ...
    # Market region and trade horizon come from config (env-overlaid), not from
    # a prompt — the interactive selectors below are used by callers that want
    # to choose them explicitly.
    selected_market_region = DEFAULT_CONFIG.get("market_region", "us")
    selected_trade_horizon = DEFAULT_CONFIG.get("trade_horizon", "short-term")
    ...
    if selected_market_region == "egypt":
        ...
        ticker_hint = "e.g. SPY, 0700.HK, BTC-USD"
        default_ticker = "SPY"
    console.print(
        create_question_box(
            "Step 1: Ticker Symbol",
            f"Enter the ticker, with exchange suffix when needed ({ticker_hint})",
            default_ticker,
        )
    )
...
def _build_run_config(selections: dict, checkpoint: bool | None) -> dict:
    ...
    # Market region / trade horizon override the config only when the caller
    # actually selected them; otherwise the env-overlaid default stands.
    for key in ("market_region", "trade_horizon"):
